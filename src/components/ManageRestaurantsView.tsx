...
import { ArrowLeft, MapPin, Plus, Store, Edit2, Trash2, PlusCircle, AlertTriangle } from "lucide-react";
...
const ManageRestaurantsView = ({ restaurants, onDelete }: ManageRestaurantsViewProps) => {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-[#1E1E1E] border border-red-500/30 p-6 rounded-2xl max-w-sm w-full shadow-2xl transform scale-100">
                  <div className="flex flex-col items-center text-center">
                      ...
                      <h3 className="text-xl font-bold text-white mb-2">Usunąć lokal?</h3>
                      <p className="text-gray-400 text-sm mb-6">
                          Czy na pewno chcesz usunąć tę restaurację? Tej operacji nie można cofnąć.
                      </p>
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
             <div className="flex items-center gap-4">
                <button onClick={() => navigate("/account")} className="text-gray-400 hover:text-white transition">
                    <ArrowLeft size={24} />
                </button>
                <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wide">Panel Partnera</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {restaurants.map((rest) => (
                <div key={rest.id} className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl overflow-hidden hover:border-gray-600 transition group relative">
                    ...
                    <div className="h-32 bg-gradient-to-r from-gray-800 to-gray-900 relative flex items-center justify-center overflow-hidden">
                        ...
                        {rest.image ? (
                            <img src={rest.image} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition duration-500" alt={rest.name} />
                        ) : (
                            <Store size={48} className="text-gray-700 group-hover:text-gray-600 transition" />
                        )}
                        
                        <div className={`absolute top-4 right-4 px-2 py-1 rounded text-[10px] font-bold uppercase border backdrop-blur-md ${
                            rest.status === "Otwarte" 
            ...
            >
                ...
                <h3 className="font-bold text-lg text-white">Dodaj kolejny lokal</h3>
