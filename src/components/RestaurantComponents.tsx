...
import MapComponent, { buildMapLink } from "./MapComponent";

// --- 1. NAGŁÓWEK RESTAURACJI (HERO) ---
export const RestaurantHero = () => {
  const mapLink =
    "https://www.google.com/maps?q=52.20064825424179,20.931893529706343";
        ...
        <div className="flex-1 flex flex-col justify-between">
          <div>
            ...
            <p className="text-gray-500 text-sm leading-relaxed mb-6">
              Najpopularniejsza sieć pizzerii na świecie.
            </p>
          </div>
...
export const MenuCard = ({ data }: { data: Pizza }) => {
  const isSpicy =
    ...
    data.sauce === "Ostry pomidorowy";
  const isVegetarian =
    data.style === "Vegetarian" || data.name.includes("Vegetariańska");
        ...
        <div className="flex flex-col items-end gap-3 min-w-[100px]">
          <div className="text-right">
            <span className="text-white font-extrabold text-2xl block tracking-tight">
              ...
              <span className="text-sm text-gray-500 font-normal">zł</span>
            </span>
    ...
    <div className="space-y-6 sticky top-6">
      <div className="bg-[#1A1A1A] rounded-2xl p-6 border border-[#2A2A2A] shadow-xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <a href="#" className="text-xs text-[#FF6B6B] font-bold hover:underline">
            Zobacz 1,204
          </a>
        </div>
      ...
      <div className="bg-[#1A1A1A] rounded-2xl border border-[#2A2A2A] overflow-hidden h-56 relative shadow-xl z-0">
        <MapComponent
          ...
          latitude={latitude}
          longitude={longitude}
          popupText={`${restaurantName} - ${address}`}
        />
